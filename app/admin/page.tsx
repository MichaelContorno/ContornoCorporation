...
export default async function AdminPage() {
  ...
  const [leadRows, intakeRows, subscriberCount, documentCount] = await Promise.all([
    ...
    DB.prepare("SELECT COUNT(*) AS count FROM subscribers WHERE active = 1").first<{ count: number }>(),
  ...
  return <BrandShell><main className="admin-page"><div className="content-wrap">
    ...
    <nav className="admin-section-nav" aria-label="Back-office sections"><a href="#attorney-intakes">Attorney intakes</a><Link href="/admin/documents">PDF document center</Link><a href="#general-inquiries">General inquiries</a></nav>
    ...
    <section className="admin-table-section" id="general-inquiries"><div className="section-heading"><div><p className="eyebrow">All service lines</p><h2>General inquiries</h2></div><span>{subscriberCount?.count ?? 0} active subscribers</span></div>
